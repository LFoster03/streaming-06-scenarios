...
from streaming.data_engineering.derived_fields_foster import enrich_message
...
from streaming.visualizations.live_visualizations_foster import (
    close_live_chart,
    init_live_chart,
    save_live_chart,
    update_live_chart,
...
OUTPUT_CSV: Final[Path] = OUTPUT_DIR / "consumed_sales_discounts.csv"
OUTPUT_DB: Final[Path] = OUTPUT_DIR / "sales.duckdb"
OUTPUT_CHART: Final[Path] = OUTPUT_DIR / "sales_chart_foster.png"
...
def load_reference_data() -> tuple[dict[str, float], dict[str, float]]:
    ...
    Returns:
        A tuple of (region_lookup, discount_lookup).
    """
    ...
    discount_lookup: dict[str, float] = {
        code: float(pct)
        for code, pct in read_csv_as_lookup(
            DISCOUNT_CODES_CSV,
    ...
    return region_lookup, discount_lookup
...
def process_message(
    ...
    *,
    ...
    enriched = enrich_message(row, region_lookup, discount_lookup)
    LOG.info(
...
def main() -> None:
    ...
    region_lookup, discount_lookup = load_reference_data()
