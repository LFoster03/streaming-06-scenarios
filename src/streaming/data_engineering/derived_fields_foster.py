"""src/streaming/data_engineering/derived_fields_foster.py.
...
def compute_discount_amount(total_price: float, discount_pct: float) -> float:
    ...
    Arguments:
        total_price: Total price before any discount.
...
def enrich_message(
    ...
    Computes total_price, discount_amount, discounted_price, tax_amount, and total
    from the raw message fields, the region lookup table, and the discount lookup table.

    As you add more derived fields, extend this function to provide them as well.
    ...
    discounted_price = round(total_price - discount_amount, 2)
    tax_amount = compute_tax_amount(discounted_price, tax_rate)
    total = round(discounted_price + tax_amount, 2)
